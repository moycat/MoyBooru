	...
	if(isset($_GET['type']) && isset($_GET['id']) and is_numeric($_GET['id']))
	{
		...
		if($type == "note")
		{
			...
			while($row = $result->fetch_assoc())
			{
				...
				if($retme['user'] == "" || $retme['user'] == null)
					...
					$user = $retme['user'];
				$set->free_result();
				echo '<a href="index.php?page=post&s=view&id='.$pid.'">'.$pid.'</a> <a href="index.php?page=history&type=note&id='.$id.'&pid='.$pid.'">'.$id.'</a>'.$row['body'].' '.$user.' '.$row['updated_at'].' <a href="#" onclick="if(confirm(\'你真的要恢复到这个历史点吗？\')){document.location=\'index.php?page=history&type=revert&id='.$id.'&pid='.$pid.'&version='.$row['version'].'\'; return false;}">恢复</a><br /><br />';
			}
			$result->free_result();
			if($count <= 0)
		...
		else if($type == "page_notes")
		{
			...
			while($row = $result->fetch_assoc())
			{
				...
				if($retme['user'] == "" || $retme['user'] == null)
					...
					$user = $retme['user'];
				$set->free_result();
				echo '<tr><td></td><td><a href="index.php?page=post&s=view&id='.$id.'">'.$id.'</a></td><td>'.$row['updated_at'].'</td><td>'.$user.'</td><td>'.$row['body'].'</td><td><a href="#" onclick="if(confirm(\'你真的要恢复到这个历史点吗？\')){document.location=\'index.php?page=history&type=revert&id='.$row['id'].'&pid='.$id.'&version='.$row['version'].'\'; return false;}">恢复</a></td></tr>';
			}
			print "</table>";
			$result->free_result();
			if($count <= 0)
		...
		else if($type == "tag_history")
		{
			...
			while($row = $result->fetch_assoc())
			{
				...
				echo '<tr><td></td><td><a href="index.php?page=post&s=view&id='.$id.'">'.$id.'</a></td><td>'.$row['updated_at'].'</td><td>'.$user.'</td><td>'.$row['tags'].'</td><td><a href="#" onclick="if(confirm(\'你真的要恢复到这个历史点吗？\')){document.location=\'index.php?page=history&type=revert_tags&id='.$id.'&version='.$row['version'].'\'; return false;}">恢复</a></td></tr>';
			}
			print "</table>";
			$result->free_result();
			if($count <= 0)
