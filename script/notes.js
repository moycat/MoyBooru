...
Note.prototype = {
	...
	showEditBox: function(e) {
		...
		inject += '<input type="submit" value="保存" name="save" id="note-save-' + this.id + '" />'
		inject += '<input type="submit" value="取消" name="cancel" id="note-cancel-' + this.id + '" />'
		inject += '<input type="submit" value="删除" name="remove" id="note-remove-' + this.id + '" />'
		inject += '<input type="submit" value="历史" name="history" id="note-history-' + this.id + '" />'
		inject += '</form>'
		inject += '</div>'
