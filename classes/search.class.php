	...
	class search
	{
		...
		function prepare_tags($search)
		{
			...
			if($g_tags != "")
			{
				...
				$query = "SELECT id, image, directory, score, rating, video, tags, owner FROM $post_table WHERE (MATCH(tags) AGAINST('$g_tags' IN BOOLEAN MODE)>0.9) $g_parent $g_owner $g_rating $parent_patch ORDER BY id DESC";
			}
